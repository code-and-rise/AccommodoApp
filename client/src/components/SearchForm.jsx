...
function SearchForm(props) {
  ...
  const [searchTerm, setSearchTerm] = useState(
    Object.keys(props).length > 0 ? props.city + ", " + props.country : ""
  );
  const [commingDate, setCommingDate] = useState(
    Object.keys(props).length > 0 ? props.commingDate : ""
  );
  const [leavingDate, setLeavingDate] = useState(
    Object.keys(props).length > 0 ? props.leavingDate : ""
  );
  const [numberOfGuests, setNumberOfGuests] = useState(
    Object.keys(props).length > 0 ? props.numberOfGuests : 1
  );
  ...
  const handleSubmit = async (e) => {
    ...
    navigate(
      `/searchResult?city=${city}&country=${country}&commingDate=${commingDate}&leavingDate=${leavingDate}&numberOfGuests=${numberOfGuests}`
    );
          ...
          <div className="card">
            <div className="card-body">
              ...
              <form onSubmit={handleSubmit} method="post">
                <div className="form-group" style={formGroupStyle}>
                  ...
                  <datalist id="options">
                    {locations.map((location, index) => (
                      <option
                        key={index}
                        value={`${location.grad}, ${location.drzava}`}
                      />
                    ))}
                  </datalist>
                </div>
                ...
                <div className="form-group" style={formGroupStyle}>
                  <label htmlFor="brojGostiju">Number of guests</label>
                  <input
                    type="number"
                    min={1}
                    className="form-control"
